...
class APrioriAlgorithm:
    ...
    '''
    def createCandidates(self, prev_L_k): #creates Ck
        C_k = []
        for i in range(len(prev_L_k)):
            for j in range(i+1, len(prev_L_k)): 
                a = list(prev_L_k[i])[:-1]; a.sort() # sort so that you can compare what was previous a (unsorted) set
                b = list(prev_L_k[j])[:-1]; b.sort()
